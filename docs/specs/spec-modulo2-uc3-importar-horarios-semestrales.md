...
|---|---|
| **Recurso** | Cualquier espacio o activo que se pueda apartar: salón, laboratorio, sala de estudio, auditorio, videobeam. |
...
- **Clase que necesita un activo ya prestado**: una clase que reserva un videobeam que un estudiante se llevó ayer por dos días no puede simplemente quitárselo; el sistema avisa del choque y son las personas responsables quienes deciden si se le pide de vuelta o la clase usa otro activo (FR-010).
...
- **FR-010**: Cuando el bloqueo académico caiga sobre un **activo** (videobeam, microscopio, kit) el sistema DEBE distinguir dos situaciones, porque un activo no se libera con solo cancelar un registro:
  - Si el activo está **apartado pero todavía no se ha recogido**, se cancela el préstamo como cualquier reserva, con el motivo `CANCELADA_POR_PRIORIDAD_ACADEMICA`, y el activo queda libre para la clase.
  - Si el activo **ya está en manos de la persona**, el sistema NO DEBE cancelar el préstamo por su cuenta: lo reporta como choque para que lo resuelvan las personas responsables, igual que hace FR-006 cuando dos clases se pisan. La prioridad académica desplaza lo que aún no ha salido del inventario; lo que ya salió hay que pedirlo de vuelta, y eso no lo puede hacer el sistema solo.
