...
- **FR-015**: Mientras dura un préstamo, el activo DEBE permanecer `EN_USO` de forma continua, sin trocearse en franjas: no se libera por las noches ni entre días, y no puede apartarlo nadie más en ningún momento de ese periodo. Solo vuelve a `DISPONIBLE` cuando se registra su devolución mediante `Reportar fecha y hora de entrega`, no al llegar la hora de vencimiento. Un activo vencido y no devuelto sigue ocupado.
