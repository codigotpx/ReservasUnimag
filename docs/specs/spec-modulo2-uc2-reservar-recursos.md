...
| Módulo 3 | Secundario | Provee las sanciones vigentes de la persona, y le reporta al Módulo 2 cuando alguien no se presenta a usar lo que apartó. |
...
- **FR-008**: El límite máximo de préstamos simultáneos DEBE ser de **3 reservas vigentes por persona**, y DEBE ser parametrizable para poder ajustarlo sin tocar el código. El mismo tope aplica a Estudiante y a Monitor, y es un cupo único: espacios y activos cuentan juntos, son 3 en total y no 3 de cada tipo. Cuenta como vigente toda reserva `CONFIRMADA` cuya franja aún no ha terminado; las `CANCELADA`, `CANCELADA_POR_PRIORIDAD_ACADEMICA` y `FINALIZADA` liberan cupo de inmediato.
...
- **FR-015**: Mientras dura un préstamo, el activo DEBE permanecer `EN_USO` de forma continua, sin trocearse en franjas: no se libera por las noches ni entre días, y no puede apartarlo nadie más en ningún momento de ese periodo. Solo vuelve a `DISPONIBLE` cuando el Módulo 3 nos manda su check-out (`Recibir check-out`), no al llegar la hora de vencimiento. Un activo vencido y no devuelto sigue ocupado.
