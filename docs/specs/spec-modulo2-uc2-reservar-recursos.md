...
**Caso de uso (diagrama)**: `Reservar recursos` (continuación opcional de `Consultar recursos`: solo ocurre si alguien, después de mirar la lista, decide apartar algo. De esta reserva cuelgan a su vez `Cancelar reserva` y `Recibir reporte de no asistencia`, que tampoco tendrían sentido sin ella)
**Prioridad global**: P1
...
| Módulo 1 | Secundario | Aporta el estado operativo del recurso (`DISPONIBLE`, `EN_USO`, `EN_MANTENIMIENTO`). La reserva y el `RESERVADO` que produce los guarda el Módulo 2, y no se le envían. |
| Módulo 3 | Secundario | Provee las sanciones vigentes de la persona; recibe la ficha de cada reserva confirmada; y le reporta al Módulo 2 cuando alguien no se presenta a usar lo que apartó y cuando un activo prestado vuelve (check-out). |

**Casos de uso relacionados**

- `Consultar recursos` — ver [spec-modulo2-uc1-consultar-recursos.md](./spec-modulo2-uc1-consultar-recursos.md)
- `Actualizar estado de los recursos` — **paso que ocurre siempre por dentro**: al confirmar la reserva, la franja o el periodo queda ocupado en el calendario del Módulo 2; no se puede saltar; ver [spec-modulo2-uc7-actualizar-estado-recursos.md](./spec-modulo2-uc7-actualizar-estado-recursos.md)
...
- `Consultar sanciones` — de ahí sale la información de sanciones que sustenta la denegación `RES-003`; ver [spec-modulo2-uc6-consultar-sanciones.md](./spec-modulo2-uc6-consultar-sanciones.md)
- `Importar horarios semestrales` — **incluye a este caso de uso** (`<<include>>`): cada clase que se importa entra como una reserva de origen académico, y esos bloqueos son los que después producen las denegaciones `RES-001` a los estudiantes; ver [spec-modulo2-uc3-importar-horarios-semestrales.md](./spec-modulo2-uc3-importar-horarios-semestrales.md)
- `Reportar información de la reserva` — **paso que ocurre siempre por dentro** (`<<include>>`): en cuanto la reserva queda confirmada, su ficha sale hacia el Módulo 3; no se puede saltar; ver [spec-modulo2-uc10-reportar-informacion-reserva.md](./spec-modulo2-uc10-reportar-informacion-reserva.md)
...
Como Estudiante, quiero apartar un recurso (aforos) disponible para una franja horaria concreta y en días (activos), y que el sistema me explique con un mensaje claro y específico cuando no puedo hacerlo, para saber exactamente qué regla estoy incumpliendo (conflicto académico, límite de préstamos alcanzado o sanción activa).
...
- **Concurrencia sobre el mismo tiempo ocupado**: si dos estudiantes solicitan a la vez el mismo recurso para tiempos que se cruzan, exactamente uno queda `CONFIRMADA` y el otro recibe `RES-004 — El recurso acaba de ser tomado`. Da igual que lo que se cruce sean dos franjas del mismo salón o dos periodos de préstamo del mismo libro: dos ocupaciones solapadas nunca pueden coexistir.
- **Activo apartado para después**: alguien puede apartar hoy un libro para recogerlo el jueves. Desde que se confirma, el libro queda ocupado durante todo el periodo que va de esa recogida a su vencimiento, así que otra persona no puede pedirlo para unas fechas que se crucen, aunque hoy el libro esté físicamente en el estante.
...
- **Activo que no se devuelve a tiempo**: llegado el vencimiento, el activo no se libera solo; sigue `EN_USO` y en manos de quien lo tiene. La mora la calcula el Módulo 3 sobre el vencimiento, y el recurso vuelve a estar disponible únicamente cuando alguien registra la devolución.
...
- **No presentación (no-show)**: al llegar la hora de la franja el recurso sigue `RESERVADO`, porque solo pasa a `EN_USO` cuando la persona se presenta. Si pasados 10 minutos el Módulo 3 reporta la no asistencia, el recurso vuelve a `DISPONIBLE` (FR-010).
...
- **FR-001**: Los usuarios DEBEN poder apartar un recurso disponible, y lo que se les pide depende de qué clase de recurso sea:
  ...
  - **Activo** (libro, microscopio, kit de dibujo, videobeam): la persona indica solo **la fecha y hora en que va a recoger el recurso**. No elige cuándo lo devuelve: la fecha de devolución la calcula el sistema aplicando el plazo del tipo (FR-012), y se la informa antes de confirmar.
- **FR-002**: El sistema DEBE validar, antes de confirmar, y en este orden: sanción activa, límite de préstamos vigentes y conflicto académico u ocupación. La sanción se comprueba contra el Módulo 3 mediante `Consultar sanciones`.
...
- **FR-004**: El sistema DEBE garantizar que no existan dos ocupaciones confirmadas solapadas sobre el mismo recurso, incluso bajo solicitudes concurrentes. Vale igual para dos franjas de un espacio, para dos periodos de préstamo de un activo y para la mezcla de ambos con un bloqueo académico: lo que no puede cruzarse es el tiempo ocupado, sea de dos horas o de siete días.
...
- **FR-008**: El límite máximo de préstamos simultáneos DEBE ser de **3 reservas vigentes por persona**, y DEBE ser parametrizable para poder ajustarlo sin tocar el código. El mismo tope aplica a Estudiante y a Monitor, y es un cupo único: espacios y activos cuentan juntos, son 3 en total y no 3 de cada tipo. Cuenta como vigente toda reserva `CONFIRMADA` de un **espacio** cuya franja aún no ha terminado, y todo préstamo de un **activo** mientras no llegue su check-out (`Recibir check-out`), aunque ya haya vencido; las `CANCELADA`, `CANCELADA_POR_PRIORIDAD_ACADEMICA`, `CANCELADA_POR_RECURSO_NO_DISPONIBLE` y `FINALIZADA` liberan cupo de inmediato.
...
- **FR-010**: El sistema DEBE liberar el recurso y dejar constancia de la ausencia **cuando el Módulo 3 le reporte que la persona no se presentó**, nunca por su cuenta; la sanción la decide y la aplica ese mismo módulo. El reporte solo se admite pasados 10 minutos, contados desde el inicio de la franja cuando se trata de un **espacio** y desde la hora de recogida cuando se trata de un **activo**: si alguien aparta un videobeam para las 14:30 y no aparece, a partir de las 14:40 el Módulo 3 puede reportarlo, y con ese aviso se cancela el periodo de préstamo completo —no solo esos diez minutos— y el activo vuelve a `DISPONIBLE`. Mientras no llegue el reporte, el recurso sigue apartado. Ver `Recibir reporte de no asistencia`.
...
- **FR-012**: El plazo máximo de préstamo de un **activo** DEBE depender de su tipo, porque no se presta igual un libro que un videobeam. El plazo NO lo define el Módulo 2: es el atributo **plazo máximo de préstamo** que el Módulo 1 fija al crear el recurso y entrega junto con el resto de la ficha, igual que el aforo o la ubicación. Se expresa en **días hábiles**, donde `0` significa que el activo se devuelve el mismo día antes de las 22:00. El sistema DEBE tomarlo de ahí para calcular la fecha y hora pactadas de devolución, y DEBE rechazar la solicitud de préstamo cuya devolución prevista lo exceda. Los valores acordados con el Módulo 1 son: Libro 7 días hábiles, Kit de dibujo 3, Videobeam 1, Microscopio 0 (uso en el laboratorio). El Módulo 2 NO DEBE mantener una copia propia de esa tabla: si el Módulo 1 cambia un plazo o crea un tipo nuevo, el valor vigente es siempre el que venga en la ficha del recurso.
...
- **FR-015**: Mientras dura un préstamo, el activo DEBE permanecer `EN_USO` de forma continua, sin trocearse en franjas: no se libera por las noches ni entre días, y no puede apartarlo nadie más en ningún momento de ese periodo. Solo deja de estar ocupado cuando el Módulo 3 nos manda su check-out (`Recibir check-out`), no al llegar la hora de vencimiento. Un activo vencido y no devuelto sigue ocupado.
- **FR-016**: El titular de un préstamo DEBE poder **renovarlo una sola vez**, sin devolver el activo ni volver a pedirlo. La renovación suma otra vez el plazo del tipo de recurso, contado **desde el vencimiento vigente** y no desde el día en que se renueva, para que nadie pierda días por renovar con antelación; el nuevo vencimiento se fija igual que el primero, a las 22:00 del día en que se cumple (FR-013). Una renovación NO consume cupo nuevo: sigue siendo el mismo préstamo y ocupa el mismo de los 3 de FR-008.
- **FR-017**: El sistema DEBE denegar la renovación cuando se dé cualquiera de estos casos: el préstamo ya fue renovado una vez; el préstamo ya venció, porque ahí lo que corre es la mora que calcula el Módulo 3 y lo que toca es devolver; el titular tiene una sanción activa; otra persona ya tiene ese mismo activo apartado para recogerlo en una fecha que el nuevo vencimiento invadiría; o el activo es de plazo `0`, de uso en sitio, que se devuelve el mismo día y no admite prórroga. En todos los casos DEBE explicarse cuál de estas razones aplica.
- **FR-018**: Al confirmar una reserva, el sistema DEBE ejecutar siempre `Reportar información de la reserva`, sin que el usuario tenga que pedirlo y sin posibilidad de omitirlo.

### Key Entities

- **Reserva**: apartado de un Recurso por un Usuario en una FranjaHoraria. Atributos: identificador, titular, recurso, franja, estado (`CONFIRMADA`, `CANCELADA`, `CANCELADA_POR_PRIORIDAD_ACADEMICA`, `CANCELADA_POR_RECURSO_NO_DISPONIBLE`, `FINALIZADA`), origen (estudiantil o académico).
- **Recurso**: espacio o activo reservable. El espacio lleva su estado por franja; el activo lo lleva por periodo de préstamo, que es continuo.
...
- **PeriodoDePrestamo**: lo que ocupa un **activo** mientras está prestado, que no cabe en una franja porque puede durar días. Atributos: fecha y hora de entrega (la que elige la persona), plazo aplicado según el tipo del recurso, fecha y hora de vencimiento (calculada, a las 22:00 del día en que se cumple el plazo), si ya se usó su única renovación y fecha y hora real de devolución, que llega desde `Recibir check-out`. Para saber si un activo está libre en una franja consultada se cruza esa franja contra este periodo, igual que se haría contra una reserva de espacio.
